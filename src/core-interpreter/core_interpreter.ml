  ...
  | (Clause(x, cl2) as cl) :: tl ->
    begin
      ...
      | Value_body(Value_function(Function_value(x', Expr(e)))) -> 
        ...
        Wddpac_graph.add_edge (x', Start_clause(Some(x')), Some(x')) graph;
      ...
      | Conditional_body(_,_,Function_value(x1, Expr(e1)),Function_value(x2, Expr(e2))) ->
        ...
        Wddpac_graph.add_edge (x1, Start_clause(Some(x1)), Some(x1)) graph;
        initialize_graph e1 graph (Some(x1));

        Wddpac_graph.add_edge (x2, Start_clause(Some(x2)), Some(x2)) graph;
...
let rec lookup_ctx context_stack ctx c =
  ...
  if (c = ctx) then context_stack
    ...
    begin
      ...
      | Some(Cond_context_var(loc, _)) -> 
        lookup_ctx (Unbounded_Stack.pop context_stack) loc c  
      ...
      if x <> var then (
        ...
        begin
          ...
          | Appl_body(xf, xv) -> 
            ...
            begin
              ...
              | Return_function(Function_value(fn_ctx, Expr(cls)), cfn, cfn_stack) ->
                  ...
                  lookup graph rx (Some(fn_ctx)) (Unbounded_Stack.push context cfn_stack)
          ...
          | Conditional_body(xc,p,f1,f2) ->
            ...
            lookup graph rx (Some(fn_ctx)) (Unbounded_Stack.push context context_stack)
...
let eval (Expr(cls)) =
  ...
  let rx = rv cls in
  let v = lookup graph rx None context_stack in
