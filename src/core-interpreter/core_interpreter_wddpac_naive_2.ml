      ...
      if x <> cur_var then
        ...
        begin
          ...
          | Value_body(v) ->
            if Stack.length lookup_stack = 1 then
              ...
              (
                ...
                (* doing lookup from current node instead of a1 - kind of a band-aid fix for when we are two levels deep *)
          ...
          | Input ->
            ...
            begin
              try
                ...
                if check_formula (substitute_value cur_formula cur_var v) then
                  (
                    (* print_endline ("current guess: " ^ (string_of_value v)); *)
                    (v, cur_formula, iota)
                  )
                else
                  raise (Evaluation_dead_end "formula unsatisfied")
          ...
          | Conditional_body(_,_,_,_) ->
            failwith "Conditional_body should have been transformed during wiring"
          | Unary_operation_body(op, var) ->
            ...
            begin
              ...
              | Unary_operator_bool_not ->
                begin
                  match v with
                  | Value_bool(b) -> (Value_bool(not b), v_formula, iota)
          ...
          | Binary_operation_body(v1, op, v2) ->
            ...
            if (Stack.length lookup_stack != 0) then
              failwith "Binary_operation_body lookup stack had more than one var on it"
            else
              (
                (* left side *)
          ...
          | _ ->
            failwith "clause body encountered not implemented yet"
        end
    end
        ...
        if param = cur_var then
          (
            if Stack.is_empty context_stack then
              (
                (* function enter parameter empty stack*)
