...
open Unbounded_context_stack_map_my_uint;;
...
let rec lookup graph var context_stack call_by_need =
  ...
  begin
            ...
            begin
              ...
              | Return_function(fn_ctx, rx, _, cfn_stack) ->
                  if call_by_need then
                    lookup graph rx (Context_table.add fn_ctx (ref (Context_var_table(xv, context_stack))) cfn_stack) call_by_need
                  else (
    ...
    | Start_clause(Some(var)) ->
      ...
      let x = Context_table.find var context_stack in
      begin
        match !x with
        | Return_type(v) ->
          v
...
and cont_uint_comp graph var context_stack call_by_need local_var curr_frame uint_val local_val comp_stack =
    ...
    begin
          ...
          begin
            ...
            | Appl_body(xf, xv) ->
              ...
              begin
                ...
                | Return_function(fn_ctx, rx, _, cfn_stack) ->
                    if call_by_need then
                      cont_uint_comp graph rx (Context_table.add fn_ctx (ref (Context_var_table(xv, context_stack))) cfn_stack) call_by_need local_var curr_frame uint_val local_val comp_stack
                    else (
                      let v = lookup graph xv context_stack call_by_need in

                      cont_uint_comp graph rx (Context_table.add fn_ctx (ref (Return_type(v))) cfn_stack) call_by_need local_var curr_frame uint_val local_val comp_stack
                    )
                | _ -> raise @@ Utils.Invariant_failure "Found incorrect definitions for function 2"
              end
            | Binary_operation_body(x1,Binary_operator_uint_plus,x2) ->

              (* let curr_frame = Context_table.add x2 context_stack curr_frame in *)
      ...
      | Conditional_clause(xc, p, x1,rx1,x2,rx2) ->
        ...
        cont_uint_comp graph rx (Context_table.add fn_ctx (ref (Return_type(v))) context_stack) call_by_need local_var curr_frame uint_val local_val comp_stack
