...
let rec env_out_transform_expr
  ...
  let (e' : On_ast.expr), (out' : 'out) =
    ...
    | On_ast.Appl (e1, e2, annots) ->
      ...
      (On_ast.Appl(e1', e2', annots), combiner out1 out2)
    | On_ast.Let (x, e1, e2) ->
      let (e1', out1) = recurse env e1 in
      let (e2', out2) = recurse env e2 in
    ...
    | On_ast.String s -> (On_ast.String s, default)
    | On_ast.Record r ->
      let kvpairs, outs =
...
let rec m_env_out_transform_expr
  ...
  let%bind ((e' : On_ast.expr), (out' : 'out)) =
    ...
    | On_ast.Appl (e1, e2, annots) ->
      ...
      return @@ (On_ast.Appl(e1', e2', annots), combiner out1 out2)
    | On_ast.Let (x, e1, e2) ->
      let%bind (e1', out1) = recurse env e1 in
      let%bind (e2', out2) = recurse env e2 in
    ...
    | On_ast.String s -> return @@ (On_ast.String s, default)
    | On_ast.Record r ->
