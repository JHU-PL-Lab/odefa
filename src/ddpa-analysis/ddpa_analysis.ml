...
sig
  ...
  val values_of_variable :
    ...
    Abstract_store_set.t * ddpa_analysis
end;;

(**
   A functor which constructs a DDPA analysis module.
*)
module Make(S : Ddpa_abstract_stores.Ops.Spec) : Analysis_sig =
struct
  module Store_ops = Ddpa_abstract_stores.Ops.Make(S);;
  module Structure_types = Ddpa_pds_structure_types.Make(Store_ops);;
  module Dynamic_pop_types =
    Ddpa_pds_dynamic_pop_types.Make(Store_ops)(Structure_types)
  ;;
  module Dynamic_pop_handler =
    Ddpa_pds_dynamic_pop_handler.Make
      (Store_ops)
      (Structure_types)
      (Dynamic_pop_types)
  ;;
  ...
  *)
  ...
  let reachable_stores
      (acl : annotated_clause)
      (stack : Structure_types.pds_continuation list)
      (analysis : ddpa_analysis)
    : Abstract_store.t Enum.t * ddpa_analysis =
    Logger_utils.lazy_bracket_log (lazy_logger `trace)
      (fun () ->
         Printf.sprintf
           "Determining reachable stores with stack %s at position %s"
           (pp_to_string
              (pp_list Structure_types.pp_pds_continuation)
              stack
           )
      ...
      (fun (values, _) ->
         pp_to_string
           (pp_concat_sep_delim "{" "}" "," Abstract_store.pp)
           values
      )
    @@ fun () ->
  ...
  let values_of_variable x acl analysis  =
    let open Structure_types in
    let (answer, analysis') =
      reachable_stores acl [Lookup_var x] analysis
    in
    (Abstract_store_set.of_enum answer, analysis')
  ;;
  ...
  let perform_closure_steps analysis =
    ...
    let new_edges_enum = Nondeterminism_monad.enum
        (
          ...
          in
          let has_values x k_opt =
            let stack =
              (Structure_types.Lookup_var x)::
              (List.of_enum @@ Option.enum k_opt)
            in
            let (values,analysis') =
              reachable_stores acl stack !analysis_ref
            in
          ...
          | Unannotated_clause(Abs_clause(x1,Abs_appl_body(x2,x3)) as cl) ->
            ...
            [%guard has_values x3 None];
            (* Get each of the function values. *)
            let (x2_values,analysis_2) =
              reachable_stores acl [Structure_types.Lookup_var x2] !analysis_ref
            in
            analysis_ref := analysis_2;
            let%bind x2_value = pick_enum x2_values in
            let%orzero Abs_value_function(fn) = store_read x2_value in
            (* Wire each one in. *)
          ...
          | Unannotated_clause(
            ...
            (* We have two functions we may wish to wire: f1 (if x2 has values
               ...
               the pattern). *)
            [ (Structure_types.Continuation_matches p, f1)
            ; (Structure_types.Continuation_antimatches p, f2)
            ]
            |> List.enum
            |> Enum.filter_map
              (fun (k,f) ->
                 if has_values x2 (Some k) then Some f else None)
