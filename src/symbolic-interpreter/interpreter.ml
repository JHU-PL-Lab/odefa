...
struct
  ...
  *)
  ...
  let rule_discovery env lookup_stack acl1 relstack recurse = 
    let open M in begin
      ...
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] << rule_discovery: " ^ (Jhupllib.Pp_utils.pp_to_string
              (Jhupllib.Pp_utils.pp_list pp_ident) lookup_stack)
              ^ (if equal_ident lookup_var env.le_first_var 
                  then "  Success!!"
                  else "  Resume")) ;
  ...
  let rule_discard lookup_stack acl1 relstack recurse = 
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_discard");
  ...
  let rule_input env lookup_stack acl1 relstack recurse = 
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_input");
      (* Lookup stack must be a singleton *)
      let%orzero [lookup_var] = lookup_stack in
  ...
  let rule_alias lookup_stack acl1 relstack recurse = 
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_alias");
  ...
  let rule_binop env lookup_stack acl1 relstack recurse = 
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_binop");
  ...
  let rule_fun_enter_para env lookup_stack acl1 relstack recurse =
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_fun_enter_para");
  ...
  let rule_fun_enter_nonlocal env lookup_stack acl1 relstack recurse =
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_fun_enter_nonlocal");
      (* Grab variable from lookup stack *)
  ...
  let rule_fun_exit env lookup_stack acl1 relstack recurse =
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_fun_exit");
      (* Grab variable from lookup stack *)
      let%orzero lookup_var :: lookup_stack' = lookup_stack in
  ...
  let rule_skip lookup_stack acl0 acl1 relstack recurse =
    let open M in begin
      (* Grab variable from lookup stack *)
  ...
  let rule_conditional_top lookup_stack acl1 relstack recurse =
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_conditional_top");
  ...
  let rule_conditional_bottom_true lookup_stack acl1 relstack recurse =
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_conditional_bottom_true");
      (* Grab variable from lookup stack *)
      let%orzero lookup_var :: lookup_stack' = lookup_stack in
      (* This must be a binding exit clause which defines our lookup
          variable. *)
  ...
  let rule_conditional_bottom_false lookup_stack acl1 relstack recurse =
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_conditional_bottom_false");
      (* Grab variable from lookup stack *)
      let%orzero lookup_var :: lookup_stack' = lookup_stack in
      (* This must be a binding exit clause which defines our lookup
          variable. *)
  ...
  let rule_handle_start_and_end lookup_stack acl1 relstack recurse = 
    let open M in begin
      lazy_logger `info (fun () -> "[" ^ string_of_int (gdc ()) ^ "] >> rule_handle_start_and_end");
  ...
  let rec lookup
    ...
    let rule_computations = (match acl1 with
        | Unannotated_clause(
            Abs_clause(Abs_var _, Abs_value_body _)) -> 
            if List.length lookup_stack = 1
            then 
              [rule_discovery env lookup_stack acl1 relstack recurse]
            else
              [rule_discard lookup_stack acl1 relstack recurse]
        | Unannotated_clause(
            Abs_clause(Abs_var _, Abs_input_body)) ->
          [rule_input env lookup_stack acl1 relstack recurse]
        | Unannotated_clause(
            Abs_clause(Abs_var _, Abs_var_body(Abs_var _))) ->
          [rule_alias lookup_stack acl1 relstack recurse]
        | Unannotated_clause(
            Abs_clause(Abs_var _, Abs_binary_operation_body(
                Abs_var _, _, Abs_var _))) -> 
          [rule_binop env lookup_stack acl1 relstack recurse]
        | Binding_enter_clause(Abs_var x'', Abs_var _, (Abs_clause(Abs_var _,Abs_appl_body(Abs_var _, _)))) ->
          let zero () = []
          in let%orzero x :: _' = lookup_stack
          in if equal_ident x x'' 
          then
            [rule_fun_enter_para env lookup_stack acl1 relstack recurse]
          else 
            [rule_fun_enter_nonlocal env lookup_stack acl1 relstack recurse]
        | Binding_exit_clause(Abs_var _, Abs_var _, (Abs_clause(Abs_var _, Abs_appl_body(Abs_var _, _)))) -> 
          [rule_fun_exit env lookup_stack acl1 relstack recurse]
        | Nonbinding_enter_clause (_, Abs_clause(_, Abs_conditional_body(Abs_var _, _, _))) ->
          [rule_conditional_top lookup_stack acl1 relstack recurse]
        | Binding_exit_clause(Abs_var _, Abs_var _, Abs_clause(_, Abs_conditional_body(Abs_var _, _, _))) -> 
          [rule_conditional_bottom_true lookup_stack acl1 relstack recurse;
           rule_conditional_bottom_false lookup_stack acl1 relstack recurse]
        | Start_clause _ | End_clause _ ->
          [rule_handle_start_and_end lookup_stack acl1 relstack recurse]
        | _ -> []) 
          @ [rule_skip lookup_stack acl0 acl1 relstack recurse]
    in let%bind m = pick @@ List.enum rule_computations in m
  ;;
